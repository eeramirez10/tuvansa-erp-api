...
const searchSchema = z.object({
  ...
  orderedAt: dateSchema.optional(), dueAt: dateSchema.optional(),
  agent: z.string().trim().min(1).max(10).optional(),
  branch: z.coerce.number().int().nonnegative().optional(),
  warehouse: z.string().trim().min(1).max(5).optional(),
  authorization: z.string().trim().min(1).max(10).optional(),
  minimumFulfillmentPercentage: z.coerce.number().min(0).max(100).optional(),
  ...
  search: RequestHandler = async (request, response, next) => {
    try {
      ...
      response.json(await this.searchOrders.execute({
        ...
        ...(query.minimumFulfillmentPercentage === undefined ? {} : { minimumFulfillmentPercentage: query.minimumFulfillmentPercentage }),
